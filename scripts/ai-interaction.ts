 ...
 */
async function callFaucet(recipient: string, amount: number): Promise<void> {
  const ethAmount = ethers.parseUnits(amount.toString(), 18);
  console.log(`Calling faucet: mint ${ethAmount} tokens to ${recipient}...`);
  const txFaucet = await tarvenCoin.faucet(recipient, ethAmount);
 ...
 */
async function callMintNFT(recipient: string, tokenData: NFT): Promise<void> {
