...
export class SceneManager {
  ...
  private createBackground() {
    ...
    this.background = this.scene.add.image(bgWidth/2, bgHeight/2, 'tavern_bg')
      .setOrigin(0.5, 0.5)
  ...
  public handlePointerDown(pointer: Phaser.Input.Pointer, moveController?: movementController) {
  ...
  private drawGrid() {
    ...
    for (let y = 0; y < this.grid.length; y++) {
      for (let x = 0; x < this.grid[0].length; x++) {
        ...
        // 设置网格线的样式
        graphics.lineStyle(1, isObstacle ? 0xff0000 : 0xffffff, isObstacle ? 0.5 : 0.3);
